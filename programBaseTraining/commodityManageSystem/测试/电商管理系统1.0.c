...
ProductList * maopao(ProductList * headProduct);
void searchProduct();
void searchCustomer();
void showsCustomer();
void showsVip();
...
void ranking();
ProductList * Smaopao(ProductList * headProduct);
    ...
    { 
		...
		float c1=1;		
   	  	...
   	  	for(i=0;i<10;i++)
   	  	{
   	  		...
   	  		{
   	  			a++;
   	  			a1=curCustomer.discountCoupon[i].discount;
			 }
			 if(curCustomer.discountCoupon[i].kind==2)
			 {
			 	b++;
   	  			b1=curCustomer.discountCoupon[i].discount;			 	
			 }
			 if(curCustomer.discountCoupon[i].kind==3)
			 {
			 	c++;
   	  			c1=curCustomer.discountCoupon[i].discount;			 	
			 }			 	  		
		 }
   	  		printf("本顾客持有的优惠券为\n");
   	  		printf("1...........%d张，折扣为%.3f\n",a,a1);
...
void setStock(ProductList* headProduct,ItemList *shoppingCart)
{
	...
	while(curItem!=NULL)
	{
		while(curProduct!=NULL)
		{
			...
			{
				if(curProduct->productInfo.stock<curItem->item.quantity)
				{
					printf("库存不足\n");
					return;
				 } 
...
//						printf("%d\n",curCount[i].countID[zhonglei].code);	
				 }
//			printf("%d\n",curCount[4].countID[1].code);			 
			zhonglei++;	 			
		}
	} fclose(fp);
//						printf("%d\n",curCount[5].countID[1].code);	
	while(p)
	{
		for(i=0;i<5;i++)
		{
...
//						printf("%d",curCount[5].countID[1].code);
					}
				}
			}
		}
...
//	printf("%d",curCount[4].countID[1].code);
	int counter;
...
//	}
	int isRight1=0;
	ProductList *p1=headProduct->next;
//	ProductList* curProduct=(ProductList*)malloc(sizeof(ProductList));
	while(p1)
	{
		for(counter=0;counter<4;counter++)
		{
			if(curCount[curCustomer->customer.salary].countID[counter].code==p1->productInfo.description.code)
			{
					ProductList* curProduct=getProduct(headProduct,curCount[curCustomer->customer.salary].countID[counter].code);
					showProductInfo(curProduct);
			}
...
int setAdministrator(int account)
{
  ...
  FILE * fp = fopen("D:\\学习材料\\软件实训\\电商管理系统\\管理员名单.txt", "r+");
  FILE * fp1 = fopen("D:\\学习材料\\软件实训\\电商管理系统\\lingshi.txt", "w+");//读写新建一个临时文件
  if(fp==NULL||fp1==NULL)
     {
      printf("打开文件失败.\n");
      exit(0);
