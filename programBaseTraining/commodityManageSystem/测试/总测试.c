 ...
 void readFile_Administrator(Administrator* headAdministrator,char *fileName)
{
	...
	if((fp=fopen(fileName,"r"))==NULL)
	{
		...
		do
		{
			...
			fread(next,sizeof(next),1,fp);											
			wandAdministrator(headAdministrator,account,password,name,gender,telNum,salary);
		}	while(!feof(fp));
}
...
void showAdministrator()
{
    ...
    { 
   	  fseek(fp,0,SEEK_SET);
   	  fread(&Admin,sizeof(ProductList),1,fp); 
...
void showsProduct()
{
    ...
    { 
   	  ...
   	  fread(&PL,sizeof(struct productInfo),1,test1); 
   	  while (feof(test1)==0)//判断是否读完 
   	  {
   	  	printf("商品编号：%d\n商品名称：%s\n商品类型：%.2d\n商品重量：%.2lf\n商品售价：%.2lf\n商品成本：%.2lf\n商品利润：%.2lf\n商品库存：%d\n",PL.description.code,PL.description.name,PL.description.kind,PL.description.weight,PL.description.price,PL.cost,PL.profit,PL.stock);
   	  	printf("***************************************************************************\n");
   	  	fread(&PL,sizeof(struct productInfo),1,test1); 
	  }
	  printf("\n..........查询完毕...............\n");	  
